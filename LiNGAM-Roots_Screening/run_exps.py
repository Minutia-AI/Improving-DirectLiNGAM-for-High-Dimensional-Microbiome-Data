...
import pandas as pd
import numpy as np
from src.data_sim import simulate_data
...
# --- single experiment ---
def run_experiment_roots_screening(p, s, n, root_frac, seed):
    
    # simulate
    X, perm, W, roots_true_orig = simulate_data(p=p, s=s, n=n, root_frac=root_frac, seed=seed)
...
def main():
    # --- job list ---
    jobs = list(product(P_LIST, S_LIST, N_LIST, ROOT_FRAC_LIST, range(RUNS)))
    ...
    results_list = Parallel(
        ...
        delayed(run_experiment_roots_screening)(p, s, n, rf, seed)
        for (p, s, n, rf, seed) in jobs
    )

    # --- assemble & save ---
    df_results = pd.DataFrame(results_list)
    df_results.to_csv("execution_time_comparison/root_screening_lingam.csv", index=False)
    print("Saved /execution_time_comparison/root_screening_lingam.csv")
